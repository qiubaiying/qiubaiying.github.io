...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall) {
        ...
        utf->utf_esp = tf->tf_esp;   //UXSTACKTOP栈上需要保存发生缺页异常时的%esp和%eip
...
​		接下来，我们需要实现组装例程，该例程将负责调用C页面错误处理程序并在原始错误指令上继续执行。这个汇编例程是将使用sys_env_set_pgfault_upcall()向内核注册的处理程序。

### 练习10

​		在lib/pfentry.S中实现_pgfault_upcall例程。有趣的部分是返回到导致页面错误的用户代码中的原始点。您将直接返回那里，而不需要通过内核返回。最困难的部分是同时切换栈和重新加载EIP。
...
_pgfault_upcall:
	...
	// 现在，C页面错误处理程序已经返回，您必须返回到陷阱时间状态。将捕获时间%eip推入捕获时间堆栈。
	//
	// Explanation:
	//  我们必须为最终返回重新执行出错的指令准备陷阱时间堆栈。不幸的是，我们不能直接从异常堆栈返回:我们不能调用“jmp”，因为这要求我们将地址加载到寄存器中，并且所有寄存器在返回后都必须有它们的捕获时间值。我们也不能从异常堆栈中调用'ret'，因为如果这样做，%esp会有错误的值。因此，我们将traptime %eip推入* traptime *堆栈!下面我们将切换到该堆栈并调用'ret'，它将把%eip恢复到它的前故障值。
	//
	// 在异常堆栈上出现递归错误的情况下，请注意，我们现在推入的单词将适合内核为我们保留的空白单词。
	//
	// 在剩下的代码中，请仔细考虑哪些寄存器可用于中间计算。您可能会发现，当寄存器作为临时空间变得不可用时，您必须以非明显的方式重新排列代码。
	//
	// LAB 4: Your code here.
    ...
    addl $0x8, %esp 
	// 恢复捕获时间寄存器。做完这个之后，你不能再修改任何通用寄存器。
	// LAB 4: Your code here.
	popal
	//从堆栈中恢复eflags。完成此操作后，您将不能再使用算术运算或任何其他修改eflags的操作。
	// LAB 4: Your code here.
	addl $0x4, %esp
    popfl
	// 切换回调整后的陷阱时间堆栈。
...
```C
// 设置页面错误处理程序函数。如果还没有，_pgfault_handler将为0。第一次注册处理程序时，我们需要分配一个异常堆栈(一个内存页，其顶部位于UXSTACKTOP)，并告诉内核在发生页面错误时调用汇编语言_pgfault_upcall例程。
void
set_pgfault_handler(void (*handler)(struct UTrapframe *utf))
{
	...
	if (_pgfault_handler == 0) {
		...
		if (sys_page_alloc(0, (void *)(UXSTACKTOP - PGSIZE), PTE_W|PTE_U|PTE_P)) {
            panic("set_pgfault_handler page_alloc failed");  	//为当前进程分配异常栈
        }   
        if (sys_env_set_pgfault_upcall(0, _pgfault_upcall)) {
            panic("set_pgfault_handler set_pgfault_upcall failed");  //系统调用，设置进程的env_pgfault_upcall属性
        }   
...
​		现在，我们已经拥有了完全在用户空间中实现写时复制fork()的内核工具。

​		我们在lib/fork.c中为您的fork()提供了一个框架。与dumbfork()类似，fork()应该创建一个新环境，然后扫描父环境的整个地址空间，并在子环境中设置相应的页面映射。关键的区别在于，虽然dumbfork()复制了页面，但是fork()最初只复制页面映射。fork()仅在其中一个环境尝试写入每个页面时才会复制它。

​		fork()的基本控制流程如下:

1. 父进程使用上面实现的set_pgfault_handler()函数将pgfault()安装为c级的页面错误处理程序。

2. 父类调用sys_exofork()来创建子环境。

3. 对于UTOP下面的地址空间中的每个可写页面或写时复制页面，父进程调用duppage, duppage应该将页上的写时复制映射到子进程的地址空间中，然后将页上的写时复制映射到它自己的地址空间中。注:这里的顺序为，在给子进程做标记之前，先在子进程身上把它标记为COW。duppage设置两个pte使页面不可写，并在“avail”字段中包含PTE_COW以区分写时复制页面和真正的只读页面。

   但是，不会以这种方式重新映射异常堆栈。相反，您需要为异常堆栈在子堆栈中分配一个新的页面。因为页面错误处理程序将执行实际的复制，而页面错误处理程序运行在异常堆栈上，所以不能在写时复制异常堆栈:谁来复制它

   fork()还需要处理存在但不能写或不能写时复制的页面。

4. 父类将用户页面错误入口点设置为与自己的页面错误入口点相似。

5. 现在，子进程已经准备好运行，因此父进程将其标记为runnable。

每当一个环境编写一个还没有编写的即写即拷的页面时，就会出现一个页面错误。下面是用户页面错误处理程序的控制流程:

1. 内核将页面错误传播到_pgfault_upcall，后者调用fork()的pgfault()处理程序。

2. pgfault()检查错误是否为写(检查错误代码中的FEC_WR)，并且页面的PTE标记为PTE_COW。如果不是,恐慌。

3. pgfault()分配一个映射到临时位置的新页面，并将出错页面的内容复制到其中。然后，故障处理程序将新页面映射到具有读/写权限的适当地址，以替代旧的只读映射。

用户级的lib/fork.c代码必须查询环境的页表，以获得上面的几个操作(例如，页面的PTE标记为PTE_COW)。内核将环境的页表映射到UVPT，正是为了这个目的。它使用了一个聪明的映射技巧，使得查找用户代码的pte变得很容易。lib/entry.S设置uvpt和uvpd，以便您可以方便地在lib/fork.c中查找页表信息。
...
pgfault(struct UTrapframe *utf)
{
	...
	// 分配一个新页面，将其映射到一个临时位置(PFTEMP)，将数据从旧页面复制到新页面，然后将新页面移动到旧页面的地址。
	// Hint:应该进行三个系统调用

	// LAB 4: Your code here.
	if ((r = sys_page_alloc(0, PFTEMP, PTE_P|PTE_U|PTE_W))) //令当前进程addr指向新分配的物理页
		panic("sys_page_alloc: %e", r);

	memmove(PFTEMP, addr, PGSIZE);  //将PFTEMP指向的物理页拷贝到addr指向的物理页

	if ((r = sys_page_map(0, PFTEMP, 0, addr, PTE_P|PTE_U|PTE_W)))  //将当前进程PFTEMP也映射到当前进程addr指向的物理页
		panic("sys_page_map: %e", r);

	if ((r = sys_page_unmap(0, PFTEMP)))  //解除当前进程PFTEMP映射
...
fork(void)
{
	...
	int r;
	if ((r = sys_page_alloc(envid, (void *)(UXSTACKTOP - PGSIZE), PTE_P|PTE_U|PTE_W)))  //为子进程分配异常栈
		panic("sys_page_alloc:%e", r);

	extern void _pgfault_upcall();
	sys_env_set_pgfault_upcall(envid, _pgfault_upcall);  //为子进程设置_pgfault_upcall

	if ((r = sys_env_set_status(envid, ENV_RUNNABLE)))  	//设置子进程为ENV_RUNNABLE状态
...
在lab 4的最后一部分中，我们将修改内核以抢占非协作环境，并允许环境之间显式地相互传递消息。

### Clock Interrupts and Preemption

​		运行user/spin测试程序。这个测试程序派生出一个子环境，一旦它接收到对CPU的控制，它就会在一个紧密的循环中永远地旋转。父环境和内核都不会重新获得CPU。就保护系统不受用户模式环境中的bug或恶意代码的影响而言，这显然不是一种理想的情况，因为任何用户模式环境都可以通过进入无限循环而永远不返回CPU，从而导致整个系统停止。为了让内核抢占一个正在运行的环境，并从它那里有力地重新夺回对CPU的控制，我们必须扩展JOS内核，以支持来自时钟硬件的外部硬件中断。

### Interrupt discipline

​		外部中断(即设备中断)被称为IRQs。有16个可能的IRQs，编号从0到15。从IRQ编号到IDT条目的映射不是固定的。pic_init在picirq.c中通过IRQ_OFFSET+15将irq0 -15映射到IDT条目IRQ_OFFSET。

​		在inc/trap.h, IRQ_OFFSET定义为decimal 32。因此，IDT条目32-47对应于IRQs 0-15。例如，时钟中断是irq0。因此,IDT [IRQ_OFFSET + 0(即IDT[32])包含内核中时钟的中断处理程序的地址。选择这个IRQ_OFFSET是为了使设备中断不与处理器异常重叠，这显然会引起混淆。(事实上，在pc运行MS-DOS的早期，IRQ_OFFSET实际上是零，这确实在处理硬件中断和处理处理器异常之间造成了巨大的混淆!)

​		与xv6 Unix相比，在JOS中我们做了一个关键的简化。外部设备中断在内核中总是禁用的(与xv6一样，在用户空间中启用)。外部中断由%eflags寄存器的FL_IF标志位控制(参见inc/mmu.h)。当这个位被设置时，外部中断被启用。虽然可以通过几种方式修改该位，但是由于我们的简化，我们将仅通过在进入和离开用户模式时保存和恢复%eflags寄存器的过程来处理它。

​		您必须确保在用户环境中运行时设置了FL_IF标志，以便当一个中断到达时，它将被传递到处理器并由您的中断代码进行处理。否则，中断将被屏蔽或忽略，直到重新启用中断。我们用引导加载程序的第一条指令来屏蔽中断，到目前为止我们还没有重新启用它们。

### 练习13

修改kern / trapentry.S和kern/trap.c来初始化IDT中的适当条目，并为IRQs 0到15提供处理程序。然后修改kern/env.c中的env_alloc()中的代码，以确保始终在启用中断的情况下运行用户环境。

还可以在sched_halt()中取消对sti指令的注释，以便空闲的cpu取消对中断的掩码。
...
​		在user/spin程序中，在子环境首次运行之后，它只是在循环中旋转，内核再也无法获得控制权。我们需要对硬件进行编程来周期性地生成时钟中断，这将迫使控制返回到内核，在内核中我们可以将控制切换到不同的用户环境。

​		对lapic_init和pic_init的调用设置了时钟和中断控制器来生成中断。现在需要编写代码来处理这些中断。

### 练习14

修改内核的trap_dispatch()函数，使它能够调用sched_yield()，以便在发生时钟中断时查找并运行不同的环境。
...
用user/spin来测试，父环境应该多次将sys_yield()子环境传递给它，但是每次都在一次时间片之后重新获得对CPU的控制，最后杀死子环境并终止。

[![Jdrjgg.jpg](https://s1.ax1x.com/2020/04/23/Jdrjgg.jpg)](https://imgchr.com/i/Jdrjgg)

### 进程间通信(IPC)

​		我们一直在关注操作系统的隔离方面，它提供了一种错觉，即每个程序都有自己的机器。操作系统的另一个重要功能是允许程序在需要的时候互相通信。让程序与其他程序交互是非常强大的。Unix管道模型就是一个典型的例子。

​		进程间通信有许多模型。即使在今天，关于哪种模式是最好的仍然存在争议。我们不讨论这个问题。相反，我们将实现一个简单的IPC机制，然后进行尝试。

### IPC in JOS

​		我们将实现几个附加的JOS内核系统调用，它们共同提供了一个简单的进程间通信机制。我们将实现两个系统调用，sys_ipc_recv和sys_ipc_try_send。然后我们将实现两个库包装器ipc_recv和ipc_send。

​		用户环境可以使用jo的IPC机制相互发送的“消息”由两个组件组成:一个32位值和一个页面映射(可选)。允许环境在消息中传递页面映射提供了一种有效的方法来传输比单个32位整数更大的数据，还允许环境轻松地设置共享内存安排。

### 发送和接收消息

​		要接收消息，环境调用sys_ipc_recv。此系统调用取消当前环境的调度，并且在收到消息之前不会再次运行它。当一个环境在等待接收消息时，任何其他环境都可以向它发送消息—不仅仅是特定的环境，也不仅仅是与接收环境有父/子安排的环境。换句话说，我们在第一部分中实现的权限检查将不适用于IPC，因为IPC系统调用是经过精心设计的，以达到“安全”的目的:一个环境不能仅仅通过发送消息就导致另一个环境发生故障(除非目标环境也有bug)。

​		要尝试发送一个值，环境调用sys_ipc_try_send，同时使用接收者的环境id和要发送的值。如果指定的环境实际上正在接收(它已经调用了sys_ipc_recv，但是还没有获得一个值)，那么send将传递消息并返回0。否则，send返回-E_IPC_NOT_RECV，表示目标环境当前不期望接收值。
...
​		当环境使用有效的dstva参数(在UTOP下面)调用sys_ipc_recv时，环境声明它愿意接收页面映射。如果发送方发送了一个页面，那么该页面应该映射到接收方的地址空间中的dstva。如果接收方已经在dstva映射了一个页面，那么前一个页面将被取消映射。

​		当一个环境调用sys_ipc_try_send具有有效srcva(低于UTOP),这意味着发送方要向接收方发送页面目前srcva映射,权限烫。IPC成功后,发送方保持原来的页面在srcva其地址空间的映射,但接收方也获得同样的物理页的映射dstva最初指定的接收方,接收方的地址空间。因此，此页面在发送方和接收方之间共享。

​		如果发送方或接收方都没有指示应该传输页面，则不传输任何页面。在任何IPC之后，内核将接收方的Env结构中的新字段env_ipc_perm设置为接收页的权限，如果没有接收页，则设置为零。

###实现IPC

### 练习15

在kern/syscall.c中实现sys_ipc_recv和sys_ipc_try_send。在执行它们之前，请阅读关于两者的评论，因为它们必须一起工作。在这些例程中调用envid2env时，应该将checkperm标志设置为0，这意味着任何环境都允许向任何其他环境发送IPC消息，而内核除了验证目标envid是有效的外，不进行任何特殊权限检查。

然后在lib/ipc.c中实现ipc_recv和ipc_send函数。

使用user/pingpong和user/primes函数来测试IPC机制。用户/质数将为每个质数生成一个新环境，直到JOS耗尽环境。您可能会发现阅读user/pri .c以了解所有的分岔和IPC在幕后进行是很有趣的。

```c
// 尝试将“值”发送到目标环境“envid”。如果srcva < UTOP，然后也发送当前映射到'srcva'的页面，以便接收者获得相同页面的重复映射。
//如果没有阻塞目标，则发送失败，返回值为-E_IPC_NOT_RECV，等待IPC。
//否则，发送成功，目标的ipc字段更新如下
//    将env_ipc_recving设置为0，以阻止未来的发送;
//    设置env_ipc_from为发送envid;
//    设置env_ipc_value为‘value’参数;
//    如果传输了一个页面，则将env_ipc_perm设置为'perm'，否则为0。
// 目标环境再次被标记为runnable，从暂停的sys_ipc_recv系统调用返回0。
// 如果发送方想要发送一个页面，但是接收方不需要，那么就不会传输页面映射，但也不会发生错误。只有在没有错误发生时才会发生ipc。
...
```c
//使用struct Env的env_ipc_recving和env_ipc_dstva字段记录希望接收的数据，将自己标记为不可运行，然后放弃CPU。
//如果'dstva' < UTOP，那么您愿意接收一页数据。'dstva'是发送页面映射到的虚拟地址。
//此函数仅在出错时返回，但系统调用最终将在成功时返回0。
// Return < 0 on error.  Errors are:
//	-E_INVAL 如果dstva < UTOP而且dstva没有页面对齐。
static int
...
```c
// 通过IPC接收一个值并返回它。
// 如果“pg”非空，则发送方发送的任何页面都将映射到该地址。
// 如果'from_env_store'非空，则将IPC发送者的envid存储在*from_env_store中。
// 如果'perm_store'非空，那么将IPC发送者的页面权限存储在*perm_store中(如果一个页面被成功传输到'pg'，则该权限为非零)。
// 如果系统调用失败，则在*fromenv和*perm中存储0(如果它们是非空的)并返回错误。
// 否则，返回发件人发送的值
//
// Hint:
// 使用'thisenv'来发现值和发送它的人。如果'pg'为空，则传递sys_ipc_recv一个它将理解为“no page”的值。(0不是正确的值，因为它是映射页面的完全有效的位置。)
int32_t
...
```C
// 发送'val'(如果'pg'不为空，则'pg'和'perm'一起发送'pg')到'toenv'。这个函数一直尝试，直到成功。
//
// Hint:
//   使用sys_yield()使其对cpu友好。
//   如果'pg'为空，则传递sys_ipc_try_send一个值，该值将被理解为“no page”。(0不是正确的值。)
void
...
使用user/pingpong函数来测试IPC机制:

[![JD9iZT.jpg](https://s1.ax1x.com/2020/04/24/JD9iZT.jpg)](https://imgchr.com/i/JD9iZT)

user/primes:

[![JD9rFg.jpg](https://s1.ax1x.com/2020/04/24/JD9rFg.jpg)](https://imgchr.com/i/JD9rFg)

make grade:

[![JD9vTO.jpg](https://s1.ax1x.com/2020/04/24/JD9vTO.jpg)](https://imgchr.com/i/JD9vTO)