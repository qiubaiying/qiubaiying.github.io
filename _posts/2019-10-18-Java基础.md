...
-  **对于3选项：一直搞不明白3选项为什么是错误的，网上的解释也都是一句话：Hash Map的哈希冲突有拉链法解决。后来才明白自己理解错误，开放地址法是在哈希表没有满的时候会采用的方法，而且基本不会起到作用，反而会越来越乱；但是Java标准库的HashMap基本上就是拉链法实现的，所以解决HashMap的哈希冲突一般采用拉链法。**
