...
#### 22.HttpServletResponse完成：设置http头标，设置cookie，设置返回数据类型，输出返回数据

#### 23.在一个基于分布式的游戏服务器系统中，不同的服务器之间，哪种通信方式是不可行的?

- 答案：管道
- 管道分为有名管道和无名管道 无名管道是只能在父子进程间通信 有名管道没有这个条件
...
- 结果：C  A   B  D  1
   - 被标明为启动类的类（即包含main()方法的类）要初始化，所以ClassB最先初始化
   - ClassB没有父类，所以初始化自身的静态变量和静态块赋值（按照声明顺序），输出“C”
   - ClassA a = new ClassA(); 初始化ClassA，输出“AB”
   - 因为类的加载信息只会在永久代里留存一份，所以c1，c2，c3实际上共用同一个ClassA.class对象，所以输出“D”
   - 静态块static{}只会执行一次，所以count=1