...
​		为了让shell工作，我们需要一种方法来键入它。QEMU已经显示了我们写入到CGA显示器和串口的输出，但到目前为止，我们只在内核监视器中获取输入。在QEMU中，在图形窗口中键入的输入将作为键盘到JOS的输入，而在控制台中键入的输入将作为串口上的字符出现。kern/console.c已经包含了键盘和自lab 1以来一直由内核监视器使用的串行驱动程序，但是现在我们需要将它们附加到系统的其余部分。

### 练习9

在kern /trap.c中调用kbd_intr来处理IRQ_OFFSET+IRQ_KBD，调用serial_intr来处理IRQ_OFFSET+IRQ_SERIAL。
...
make run-testkbd 测试一下：

[![YSvdgI.jpg](https://s1.ax1x.com/2020/05/03/YSvdgI.jpg)](https://imgchr.com/i/YSvdgI)

## The Shell

​		运行make run-icode 或 make run-icode-nox。这将运行内核并启动user/icode.code执行init，它将把控制台设置为文件描述符0和1(标准输入和标准输出)。然后它会衍生出sh，也就是壳层。你应该能够运行以下命令:
...
​		注意，用户库例程cprintf直接打印到控制台，而不使用文件描述符代码。这对于调试来说很好，但是对于导入其他程序来说就不太好了。要将输出输出到特定的文件描述符(例如，1，标准输出)，请使用fprintf(1，“…”，…)。printf("…"，…)是将数据打印到FD 1的捷径。有关示例，请参见user/ lsfdc。

### 练习10

shell不支持I/O重定向。我们最好运行sh <script，而不是像上面那样手工输入脚本中的所有命令。为<添加I/O重定向到user/sh.c。

通过在shell中键入sh <script来测试我们的实现

运行 make run-testshell来测试您的shell。testshell只是将上面的命令(也可以在fs/testshell.sh中找到)提供给shell，然后检查输出是否与fs/testshell.key匹配。
...
[![YSOc59.jpg](https://s1.ax1x.com/2020/05/03/YSOc59.jpg)](https://imgchr.com/i/YSOc59)