...
* 相关子查询就是要循环执行多次的子查询
    * ```sql
        ...
        AND empno != 7499;
        ```
        括号中即相关子查询，WHERE语句每过滤一条数据，子查询就会执行一次
* 因为MyBatis等持久层框架开启了缓存功能，其中一级缓存就会保存子查询的结果，所以可以放心使用子查询
* 结论：SQL控制台不要使用子查寻，在持久层框架中则可以使用
...
![redo-undo](https://ahogek-oss.oss-cn-hangzhou.aliyuncs.com/blog-img/Screenshot%20from%202020-05-01%2016-28-42.png)

#### 事务机制(Transaction)

* 事物是一个或多个SQL语句组成的整体，要么全部执行成功，要么全部执行失败
...
![transaction](https://ahogek-oss.oss-cn-hangzhou.aliyuncs.com/blog-img/Screenshot%20from%202020-05-01%2016-39-13.png)

#### 事务的ACID属性

![ACID](https://ahogek-oss.oss-cn-hangzhou.aliyuncs.com/blog-img/Screenshot%20from%202020-05-01%2016-42-01.png)
...
* 隔离性要求事务不受其他并发事务的影响，默认情况下A事务，只能看到日志中该事务的相关数据

#### 持久性

* 事务一旦提交，结果便是永久性的。即便发生宕机，仍然可以依靠事务日志完成数据的持久化

**[分布式事务](https://juejin.im/post/5b5a0bf9f265da0f6523913b#heading-20)**