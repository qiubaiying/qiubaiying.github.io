...
layout:     post
title:      超导的物理图像
subtitle:   一个有趣的类比，cooper对与自旋子
date:       2019-04-29
author:     JPZhuang
header-img: img/post-cooper-pair.jpg
catalog: true
tags:
    - superconductor
---
...
> Ref: Piers Coleman[《Introduction to Many-Body Physics》](http://www.jiujitsubrotherhood.com/the-biggest-lie-in-jiu-jitsu/)
...
$H_{M F T}=\sum_{\mathbf{k} \sigma} \epsilon_{\mathbf{k}} c_{\mathbf{k} \sigma}^{\dagger} c_{\mathbf{k} \sigma}+\sum_{\mathbf{k}}\left[\overline{\Delta} c_{-\mathbf{k} \downarrow} c_{\mathbf{k} \uparrow}+c_{\mathbf{k} \uparrow}^{\dagger} c_{-\mathbf{k} \downarrow}^{\dagger} \Delta\right]+\frac{V}{g_{0}} \overline{\Delta} \Delta$

其中能隙$\Delta=-g_{0}\int_{\left|\epsilon_{\mathrm{k}}\right|<\omega_{D}}\frac{d^{3} k}{(2 \pi)^{3}}\left\langle c_{-\mathbf{k} \downarrow}c_{\mathbf{k}\uparrow}\right\rangle$与配对的密度有关
...
- BCS理论的准粒子激发
- 路径积分形式
- 南部-戈尔科夫（Nambu–Gor’kov）格林函数
- 扭曲相位：超流


## 各项公式的物理含义

- **Pair creation**：$\overline{\Delta} c_{-\mathbf{k} \downarrow} c_{\mathbf{k} \uparrow}$表示把两个电子凝聚成一个电子对$e^{-}+e^{-} \rightleftharpoons \mathrm{pair}^{2-}$
- **hole creation**：$c_{-\mathrm{k} \downarrow}=h_{\mathrm{kl}}^{\dagger}$
- **Andreev reflection**：$\left(h_{\mathrm{k} \uparrow}^{\dagger} \overline{\Delta}\right) c_{\mathrm{k} \uparrow}$表示一个电子被散射成一个cooper对和一个空穴$e^{-} \rightleftharpoons \mathrm{pair}^{2-}+h^{+}$
