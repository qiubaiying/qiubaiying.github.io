...
首先，我们要知道，Linux中进程之间是有隔离的，而且每个进程的进程空间都会分为“用户控件”和“内核空间”，对应着“用户态”和“内核态”，而“系统调用”则是用户空间访问内核空间的唯一方式，系统调用主要通过如下两个函数实现：
- copy_from_user() //将数据从用户空间拷贝到内核空间
- copy_to_user() //将数据从内核空间拷贝到用户空间

接下来，我们就可以研究传统IPC通讯的原理了，如下图所示：
![](https://raw.githubusercontent.com/OldJii/pic/master/5b391412f26f4a4aa5d68a1db0dd0a04.jpg)
...
## Android多进程引发的问题
在介绍AndroidIPC之前，先说一下为什么需要这些方式，原有的Android通讯机制或者LinuxIPC满足不了Android多线程吗？
...
**优点**：对数据进行安全的封装；提供统一的存取数据的接口供其他进程调用

**场景**：一对多的进程间共享数据

**用法**：https://lrh1993.gitbooks.io/android_interview_guide/content/android/basis/ContentProvider.html
...
**关于序列化**：http://note.youdao.com/noteshare?id=5521000160fbda9125b03909587fb2b6
...
想了解Handler机制，可以看我的有道云笔记：http://note.youdao.com/noteshare?id=5eb43fdb54d3c64cb933cb5280ba5201

**原理**：底层实现是AIDL

**优点**：线程安全，不支持并发（既可以说是优点也可以说是缺点）；封装AIDL，使用简单，不需要AIDL文件；支持实时通讯

**缺点**：串行处理客户端发来的消息，服务端不存在并发情况；数据通过Message传递所以只能传递Bundle支持的数据类型

**场景**：低并发的一对多即使通讯

**使用**：https://www.jianshu.com/p/52ec31fcf0a6
...
详情参考：http://note.youdao.com/noteshare?id=e1854dc91b709f71f71c397ad5ea2217
...
详情请参考：http://jiguankai.cn/2019/09/17/Android手机从开机到APP启动经过的流程/
...
**直接使用**：https://www.jianshu.com/p/2d6ddd6a3399

### Binder底层原理
```
先回顾一下传统IPC的原理

然后讲动态内核可加载模块 和 内存映射，明白“Binder如何跟别人不同”

然后再写出BinderIPC的流程

最后就“一次拷贝”这个点提一句: “这就是Android选择Binder的原因之一”
```
...
#### 动态内核可加载模块
Linux的动态内核可加载模块（LKM）是一段具有独立功能的程序，它可以被单独编译，到那时不能单独运行，该模块在运行时被链接到内核作为内核的一部分运行。这样Android系统就可以通过动态添加一个内核模块运行在内核空间，用户进程之间则通过这个内核模块作为桥梁来实现通讯
...
如下图所示：
![](https://raw.githubusercontent.com/OldJii/pic/master/20200108145347.png)
...
![](https://raw.githubusercontent.com/OldJii/pic/master/20200108150804.png)
...
更多关于Binder源码的内容请参考：http://gityuan.com/2015/11/01/binder-driver/
...
![](https://raw.githubusercontent.com/OldJii/pic/master/20200108154444.png)
