...
* 插件支持其他数据摄取协议，例如Graphite，collected和OpenTSDB。
...
* InfluxDB的开源版本在单个节点上运行。如果您需要高可用性来消除单点故障，请考虑使用InfluxDB企业版。
