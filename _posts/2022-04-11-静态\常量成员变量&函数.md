...
{
	value = 0;		//常量成员函数在执行过程中，不能修改它所作用的对象的值。而这条语句修改了主函数中某对象的成员变量的值，不能这么写！
	func();			//常量成员函数**不可以**调用类内非常量成员函数，因此也不能这么写！
}
...
* 如果用对象做参数，生成该参数又需要调用**拷贝构造函数**，会构成时间和空间上的开销。
* 如果用指针做参数，可以是可以，但是函数是在有点难看..
* 一般会用常引用来做函数的参数，这样高效好看：
...
void PrintObj(const Sample & o) {			//使用const是为了避免函数形参修改后，实参o也跟着变的情况发生
	...			//这里就不会调用拷贝构造函数。我们在这个函数里就可以访问o(实参)这个值，且o也不会被修改
}
```
