...
* substr(a, b): 复制子字符串，从指定位置a开始，截取长度为b的字符串
* stoi(): 将数字字符转化为int输出，string to int嘛。需要注意的是，stoi()会对参数字符串进行**范围判断**，默认范围是在int的范围内[ -2147483648, 2147483647]的，如果超出范围的话则会runtime error.
```CPP
int main()
{
